...
export class AtlasRenderer {
  ...
  private drawRecentCaptures(snapshot: SimulationSnapshot): void {
    ...
    g.clear();
    const captureTime = snapshot.recentCaptureTime;
    const captureSide = snapshot.recentCaptureSide;
    if (!captureTime || !captureSide || captureTime.length !== snapshot.width * snapshot.height) return;

    for (let y = 0; y < snapshot.height; y++) {
      for (let x = 0; x < snapshot.width; x++) {
        if ((x - y) % CAPTURE_HATCH_SPACING !== 0) continue;
        const i = y * snapshot.width + x;
        const capturedAt = captureTime[i];
